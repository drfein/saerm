...
- `embedding_jobs`: which model/layer/dataset combinations to cache. Each job describes the model id, target layer, dataset key, tokenizer override, and sampling controls. Provide `prompt_field`, `chosen_field`, and `rejected_field` so the cache can build a full conversation (user prompt + assistant reply) for both candidates and render it with `tokenizer.apply_chat_template`. Every dataset row yields two embeddings (chosen/rejected) plus a `records` table that maps each vector back to its example index, choice label, and rendered text. A verification report (shape, variance, uniqueness) is written alongside the metadata to catch degenerate dumps early.
- `sae_jobs`: sparse autoencoder jobs that reference an embedding job id and list training hyperparameters.
  ...
  - Advanced SAE knobs mirror the Matryoshka/aux-K implementation: `activation`, `aux_k`/`aux_loss_coef`, `prefix_lengths` for Matryoshka loss, `batch_topk_threshold_lr`, `dead_neuron_threshold_steps`, and `grad_clip_norm`. Batch Top-K sparsity is always enabled; tune the threshold LR to control how quickly the EMA adapts.
...
- Spot-check embedding caches at any point with `scripts/verify_embeddings.py --config config.yaml [--job-id JOB]`; it reuses the same verification used during caching and exits non-zero when issues are detected.
