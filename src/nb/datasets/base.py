...
class ProbeDataset(ABC):
    ...
    @abstractmethod
    def _make_contrastive_pair(self, raw_example: Any, tokenizer: Any) -> Optional[ContrastivePair | List[ContrastivePair]]:
        """Convert raw example to contrastive pair(s) for probe building.
        ...
        Returns:
            ContrastivePair, List[ContrastivePair], or None if example should be skipped
        """
        pass
    ...
    def _compute_splits(self) -> None:
        ...
        If probe_size is 0, all examples go to test (for cross-dataset eval).
        Otherwise, ensures at least 20% of examples are reserved for testing.
        ...
        # Special case: probe_size=0 means use ALL for evaluation
        if self.probe_size == 0:
            self._probe_indices = []
            all_indices = list(range(n_total))
            if self.max_test_examples is not None:
                all_indices = all_indices[:self.max_test_examples]
            self._test_indices = all_indices
            logger.info("Split: 0 probe, %d test (all for evaluation)", len(all_indices))
            return
    ...
    def get_probe_pairs(self, tokenizer: Any) -> List[ContrastivePair]:
        ...
        for idx in self._probe_indices:
            res = self._make_contrastive_pair(self._raw_data[idx], tokenizer)
            if res is None:
                continue
...
def normalize_text(text: Any, role: Optional[str] = None) -> str:
    """Normalize text from various formats (string, list of dicts, list of strings).
    
    Args:
        text: The text to normalize
        role: If text is a list of message dicts, only extract content for this role
              ('user' or 'assistant'). If None, extracts the last message.
              
    Returns:
        Normalized string or empty string if no valid text found.
    """
    ...
    if isinstance(text[0], dict):
        if role:
            # Find the last message matching the role
    ...
    # List of strings: take the last one
