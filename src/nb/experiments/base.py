...
class ExperimentResults:
    ...
    baseline_per_example: Optional[Dict[str, List[int]]] = None
    """Binary indicators (0/1) for baseline evaluation"""
    
    nulled_per_example: Optional[Dict[str, List[int]]] = None
    """Binary indicators (0/1) for nulled evaluation"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "baseline_per_example": self.baseline_per_example,
            "nulled_per_example": self.nulled_per_example,
        }
    ...
    def load(cls, path: Path) -> "ExperimentResults":
        ...
        return cls(
            ...
            baseline_per_example=data.get("baseline_per_example"),
            nulled_per_example=data.get("nulled_per_example"),
        )
...
class BiasExperiment(ABC):
    ...
    def _probe_dir(self) -> Path:
        return Path(self.config.artifacts_dir) / "probes" / self.config.bias_type / self.config.name
    ...
    def _save_probe_artifacts(
        ...
        if not self.config.save_probe or self.probe is None:
            return
        ...
        if self.config.extra.get("save_probe_activations", False) and activations is not None:
            torch.save(activations, probe_dir / "activations.pt")

        logger.info("Saved probe artifacts to %s", probe_dir)
    ...
    def _compute_metrics(
        ...
        Returns:
            Tuple of (metrics_dict, per_example_indicators)
        """
        pass
    ...
    def load_dataset(self) -> None:
        ...
        for cross-dataset generalization experiments. In this case:
        - Probe is built from probe_source with probe_size examples
        - ALL examples from the main dataset are used for evaluation (no split)
        """
        # Check for cross-dataset setup BEFORE creating eval dataset
        probe_source = self.config.extra.get("probe_source")
        
        if probe_source:
            # Cross-dataset: use ALL eval examples (no split)
            logger.info("Cross-dataset mode: will use ALL eval examples")
            original_probe_size = self.config.probe_size
            self.config.probe_size = 0  # No probe split for eval dataset
            self.dataset = self._create_dataset()
            self.config.probe_size = original_probe_size  # Restore for probe dataset
    ...
    def build_probe(self) -> Dict[str, Any]:
        ...
        activations = None
        if self.config.extra.get("save_probe_activations", False):
            self.probe, metadata, activations = build_probe_direction(
                model=self.model,
        ...
        self._save_probe_artifacts(metadata, activations)
    ...
    def evaluate(self) -> ExperimentResults:
        ...
        logger.info("Running evaluation (baseline + nulled in single pass)...")
        ...
        baseline_metrics, baseline_per_example = self._compute_metrics(baseline_organized, eval_examples)
        
        nulled_organized = self._organize_rewards(nulled_rewards, text_meta, n_eval)
        nulled_metrics, nulled_per_example = self._compute_metrics(nulled_organized, eval_examples)
        ...
        self.results = ExperimentResults(
            ...
            nulled_per_example=nulled_per_example,
        )
